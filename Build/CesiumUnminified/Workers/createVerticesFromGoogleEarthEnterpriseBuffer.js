...
define([
  ...
  function processBuffer(
    ...
    for (quad = 0; quad < 4; ++quad) {
      ...
      var originX = _Math.CesiumMath.toRadians(
        dv.getFloat64(offset, true) * 180.0
      );
      offset += sizeOfDouble;

      var originY = _Math.CesiumMath.toRadians(
        dv.getFloat64(offset, true) * 180.0
      );
      offset += sizeOfDouble;

      var stepX = _Math.CesiumMath.toRadians(
        dv.getFloat64(offset, true) * 180.0
      );
      var halfStepX = stepX * 0.5;
      offset += sizeOfDouble;

      var stepY = _Math.CesiumMath.toRadians(
        dv.getFloat64(offset, true) * 180.0
      );
        ...
        if (
          ...
          if (index === -1) {
            quadBorderPoints.push(
              Cartesian2.Cartographic.clone(scratchCartographic)
            );
  ...
  function addSkirt(
    ...
    for (var j = 0; j < count; ++j) {
      ...
      Transforms.Matrix4.multiplyByPoint(
        skirtOptions.toENU,
        pos,
        scratchCartesian
      );
