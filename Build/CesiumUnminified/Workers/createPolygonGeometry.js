...
define([
  ...
  "./GeometryPipeline-869c813c",
  "./EncodedCartesian3-1e19ea62",
  "./IndexDatatype-66029622",
  "./IntersectionTests-d561f371",
  "./Plane-1577f476",
  "./GeometryOffsetAttribute-d746452d",
  "./VertexFormat-4d8b817a",
  "./GeometryInstance-3c1cb37f",
  "./arrayRemoveDuplicates-d1f2f54f",
  "./BoundingRectangle-163e2ea9",
  "./EllipsoidTangentPlane-38b58c36",
  "./ArcType-dc1c5aee",
  "./EllipsoidRhumbLine-802489e5",
  "./PolygonPipeline-b54772e1",
  "./PolygonGeometryLibrary-edb0ac1f",
  "./EllipsoidGeodesic-fadac0dc",
], function (
  when,
  Check,
  _Math,
  ...
  function computeAttributes(options) {
    ...
    if (
      ...
      if (stRotation !== 0.0) {
        ...
        textureMatrix = Transforms.Matrix3.fromQuaternion(
          rotation,
          textureMatrix
        );
        ...
        textureMatrix = Transforms.Matrix3.clone(
      ...
      for (var i = 0; i < length; i += 3) {
        ...
        if (vertexFormat.st) {
          ...
          var stx = _Math.CesiumMath.clamp(
            st.x / boundingRectangle.width,
            0,
            1
          );
          var sty = _Math.CesiumMath.clamp(
        ...
        if (
          ...
          if (wall) {
            if (i + 3 < length) {
              var p1 = Cartesian2.Cartesian3.fromArray(
                flatPositions,
                i + 3,
                p1Scratch
              );
            ...
            if (vertexFormat.tangent || vertexFormat.bitangent) {
              ...
              tangent = Cartesian2.Cartesian3.cross(
                Cartesian2.Cartesian3.UNIT_Z,
                normal,
                tangent
              );
    ...
    if (options.extrude && when.defined(options.offsetAttribute)) {
      ...
      if (
        ...
        if ((top && bottom) || wall) {
          offsetAttribute = GeometryOffsetAttribute.arrayFill(
            offsetAttribute,
            1,
            0,
            size / 2
  ...
  function computeRectangle(
    ...
    if (arcType === ArcType.ArcType.RHUMB) {
      return Cartesian2.Rectangle.fromCartesianArray(
        positions,
        ellipsoid,
        result
      );
  ...
  function createGeometryFromPositionsExtruded(
    ...
    var windingOrder = PolygonPipeline.PolygonPipeline.computeWindingOrder2D(
      positions2D
    );
    ...
    for (i = 0; i < holes.length; i++) {
      ...
      windingOrder = PolygonPipeline.PolygonPipeline.computeWindingOrder2D(
        positions2D
      );
  ...
  function PolygonGeometry(options) {
    ...
    var vertexFormat = when.defaultValue(
    ...
    this._arcType = when.defaultValue(
      options.arcType,
  ...
  var scratchEllipsoid = Cartesian2.Ellipsoid.clone(
    Cartesian2.Ellipsoid.UNIT_SPHERE
  );
  ...
  PolygonGeometry.unpack = function (array, startingIndex, result) {
    ...
    var ellipsoid = Cartesian2.Ellipsoid.unpack(
      array,
      startingIndex,
      scratchEllipsoid
    );
    ...
    result._ellipsoid = Cartesian2.Ellipsoid.clone(
      ellipsoid,
      result._ellipsoid
    );
  ...
  function createPolygonGeometry(polygonGeometry, offset) {
    ...
    polygonGeometry._ellipsoid = Cartesian2.Ellipsoid.clone(
