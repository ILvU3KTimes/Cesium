...
define([
  ...
  "./PolylineVolumeGeometryLibrary-60333c48",
  "./EllipsoidGeodesic-fadac0dc",
  "./PolylinePipeline-9a630005",
], function (
  when,
  Check,
  _Math,
  ...
  function computeAttributes(
    ...
    if (vertexFormat.tangent || vertexFormat.bitangent) {
      try {
        geometry = GeometryPipeline.GeometryPipeline.computeTangentAndBitangent(
          geometry
        );
      } catch (e) {
        oneTimeWarning(
  ...
  function PolylineVolumeGeometry(options) {
    ...
    this._cornerType = when.defaultValue(
      options.cornerType,
    ...
    var numComponents =
      1 + positions.length * Cartesian2.Cartesian3.packedLength;
  ...
  PolylineVolumeGeometry.pack = function (value, array, startingIndex) {
    ...
    for (
      i = 0;
      i < length;
      ++i, startingIndex += Cartesian2.Cartesian2.packedLength
    ) {
  ...
  PolylineVolumeGeometry.createGeometry = function (polylineVolumeGeometry) {
    ...
    shape2D = PolylineVolumeGeometryLibrary.PolylineVolumeGeometryLibrary.removeDuplicatesFromShape(
      shape2D
    );
    ...
    if (
      PolygonPipeline.PolygonPipeline.computeWindingOrder2D(shape2D) ===
      PolygonPipeline.WindingOrder.CLOCKWISE
    ) {
      shape2D.reverse();
    }
    var boundingRectangle = BoundingRectangle.BoundingRectangle.fromPoints(
      shape2D,
      brScratch
    );
