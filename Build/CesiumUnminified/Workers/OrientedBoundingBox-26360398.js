...
define([
  ...
  function OrientedBoundingBox(center, halfAxes) {
     ...
     */
    this.center = Cartesian2.Cartesian3.clone(
      when.defaultValue(center, Cartesian2.Cartesian3.ZERO)
    );
    /**
     ...
     */
    this.halfAxes = Transforms.Matrix3.clone(
      when.defaultValue(halfAxes, Transforms.Matrix3.ZERO)
    );
  }
  ...
  OrientedBoundingBox.fromPoints = function (positions, result) {
    ...
    var meanPoint = Cartesian2.Cartesian3.clone(
      positions[0],
      scratchCartesian1
    );
    for (i = 1; i < length; i++) {
    ...
    for (i = 0; i < length; i++) {
      p = Cartesian2.Cartesian3.subtract(
        positions[i],
        meanPoint,
        scratchCartesian2
      );
    ...
    var rotation = Transforms.Matrix3.clone(
  ...
  function fromPlaneExtents(
    ...
    centerOffset = Transforms.Matrix3.multiplyByVector(
      halfAxes,
      centerOffset,
      centerOffset
    );
  ...
  OrientedBoundingBox.fromRectangle = function (
    ...
    if (rectangle.width <= _Math.CesiumMath.PI) {
      ...
      var tangentPlane = new EllipsoidTangentPlane.EllipsoidTangentPlane(
        tangentPoint,
        ellipsoid
      );
    ...
    var planeXAxis = Cartesian2.Cartesian3.cross(
      planeNormal,
      planeYAxis,
      scratchPlaneXAxis
    );
  ...
  OrientedBoundingBox.intersectPlane = function (box, plane) {
    ...
    var distanceToPlane =
      Cartesian2.Cartesian3.dot(normal, center) + plane.distance;
  ...
  OrientedBoundingBox.distanceSquaredTo = function (box, cartesian) {
    ...
    var offset = Cartesian2.Cartesian3.subtract(
      cartesian,
      box.center,
      scratchOffset
    );
