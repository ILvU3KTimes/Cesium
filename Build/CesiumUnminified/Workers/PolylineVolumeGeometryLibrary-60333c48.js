...
define([
  ...
  function addPosition(
    ...
    west = Transforms.Matrix4.multiplyByPointAsVector(
      transform,
      negativeX,
      west
    );
    ...
    transform = Transforms.Matrix4.multiplyTransformation(
      transform,
      Transforms.Matrix4.fromRotationTranslation(
        rotationZ,
        heightCartesian,
        translation
      ),
    ...
    for (var j = 0; j < repeat; j++) {
      for (var i = 0; i < shape.length; i += 3) {
        finalPosition = Cartesian2.Cartesian3.fromArray(
          shape,
          i,
          finalPosition
        );
  ...
  function computeRoundCorner(
    ...
    if (granularity > 0) {
      ...
      for (var i = 0; i < granularity; i++) {
        startPoint = Transforms.Matrix3.multiplyByVector(
          m,
          startPoint,
          startPoint
        );
  ...
  PolylineVolumeGeometryLibrary.computePositions = function (
    ...
    for (var i = 1; i < length - 1; i++) {
      ...
      forward = Cartesian2.Cartesian3.subtract(nextPosition, position, forward);
      forward = Cartesian2.Cartesian3.normalize(forward, forward);
      cornerDirection = Cartesian2.Cartesian3.add(
        forward,
        backward,
        cornerDirection
      );
      ...
      var doCorner = !_Math.CesiumMath.equalsEpsilon(
        Math.abs(
          Cartesian2.Cartesian3.dot(forwardProjection, backwardProjection)
        ),
        1.0,
      ...
      if (doCorner) {
        ...
        if (leftIsOutside) {
          ...
          scratch2Array[0] = Cartesian2.Cartesian3.clone(
            previousPosition,
            scratch2Array[0]
          );
          scratch2Array[1] = Cartesian2.Cartesian3.clone(
            start,
            scratch2Array[1]
          );
          ...
          if (
            ...
            cornerDirection = Cartesian2.Cartesian3.negate(
              cornerDirection,
              cornerDirection
            );
            finalPositions = addPosition(
