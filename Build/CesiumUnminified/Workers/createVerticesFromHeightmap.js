...
define([
  ...
  "./AttributeCompression-8a649799",
  "./IntersectionTests-d561f371",
  "./Plane-1577f476",
  "./WebMercatorProjection-75bbddfc",
  "./createTaskProcessorWorker",
  "./EllipsoidTangentPlane-38b58c36",
  "./OrientedBoundingBox-26360398",
  "./TerrainEncoding-adb973b4",
], function (
  when,
  Check,
  _Math,
  ...
  HeightmapTessellator.computeVertices = function (options) {
    ...
    var ellipsoid = when.defaultValue(
      options.ellipsoid,
      Cartesian2.Ellipsoid.WGS84
    );
    ...
    relativeToCenter = hasRelativeToCenter
      ? relativeToCenter
      : Cartesian2.Cartesian3.ZERO;
    var exaggeration = when.defaultValue(options.exaggeration, 1.0);
    var includeWebMercatorT = when.defaultValue(
      options.includeWebMercatorT,
      false
    );
    ...
    var fromENU = Transforms.Transforms.eastNorthUpToFixedFrame(
      relativeToCenter,
      ellipsoid
    );
    var toENU = Transforms.Matrix4.inverseTransformation(
      fromENU,
      matrix4Scratch
    );
    ...
    for (var rowIndex = startRow; rowIndex < endRow; ++rowIndex) {
      ...
      for (var colIndex = startCol; colIndex < endCol; ++colIndex) {
        ...
        Cartesian2.Cartesian3.minimumByComponent(
          cartesian3Scratch,
          minimum,
          minimum
        );
        Cartesian2.Cartesian3.maximumByComponent(
          cartesian3Scratch,
          maximum,
          maximum
        );
    ...
    var aaBox = new EllipsoidTangentPlane.AxisAlignedBoundingBox(
      minimum,
      maximum,
      relativeToCenter
    );
  ...
  */
  ...
  (function () {
    ...
    var LercDecode = (function () {
      ...
      CntZImage.defaultNoDataValue = -3.4027999387901484e38; // smallest Float32 value
      ...
      CntZImage.decode = function (input, options) {
        ...
        var skipMask =
          options.encodedMaskData || options.encodedMaskData === null;
        var parsedData = parse(input, options.inputOffset || 0, skipMask);

        var noDataValue =
          options.noDataValue !== null
            ? options.noDataValue
            : CntZImage.defaultNoDataValue;
        ...
        if (options.returnEncodedMask && parsedData.mask) {
          result.encodedMaskData = parsedData.mask.bitset
            ? parsedData.mask.bitset
            : null;
        }
      ...
      var parse = function (input, fp, skipMask) {
        ...
        if (data.fileIdentifierString.trim() !== "CntZImage") {
          throw (
            "Unexpected file identifier string: " + data.fileIdentifierString
          );
        }
        fp += 10;
        ...
        if (!skipMask) {
          ...
          if (data.mask.numBytes > 0) {
            var bitset = new Uint8Array(
              Math.ceil((data.width * data.height) / 8)
            );
            ...
            (data.mask.numBytes | data.mask.numBlocksY | data.mask.maxValue) ===
            0
          ) {
            // Special case, all nodata
            data.mask.bitset = new Uint8Array(
              Math.ceil((data.width * data.height) / 8)
            );
          }
        }
        ...
        var actualNumBlocksX =
          numBlocksX + (data.width % numBlocksX > 0 ? 1 : 0);
        var actualNumBlocksY =
          numBlocksY + (data.height % numBlocksY > 0 ? 1 : 0);
        ...
        for (var blockY = 0; blockY < actualNumBlocksY; blockY++) {
          for (var blockX = 0; blockX < actualNumBlocksX; blockX++) {
            ...
            if (block.encoding === 0) {
              ...
              var dataBytes = Math.ceil(
                (block.numValidPixels * block.bitsPerPixel) / 8
              );
      ...
      var unstuff = function (
        ...
        var numInvalidTailBytes =
          src.length * 4 - Math.ceil((bitsPerPixel * numPixels) / 8);
    ...
    var Lerc2Decode = (function () {
          ...
          if (lutArr) {
            for (o = 0; o < numPixels; o++) {
              ...
              if (bitsLeft >= bitsPerPixel) {
                ...
                bitsLeft = 32 - missingBits;
                n |=
                  (buffer & ((1 << missingBits) - 1)) <<
                  (bitsPerPixel - missingBits);
                bitPos = missingBits;
              }
              dest[o] = lutArr[n];
            }
      ...
      var Lerc2Helpers = {
        ...
        readHeaderInfo: function (input, data) {
          ...
          var headerInfo = {};
          headerInfo.fileIdentifierString = String.fromCharCode.apply(
            null,
            fileIdView
          );
          if (headerInfo.fileIdentifierString.lastIndexOf("Lerc2", 0) !== 0) {
            throw (
              ...
              headerInfo.fileIdentifierString
            );
          }
          ptr += 6;
          ...
          if (fileVersion >= 3) {
            ...
            checksum = this.computeChecksumFletcher32(
              new Uint8Array(input, ptr - keyLength, headerInfo.blobSize - 14)
            );
        ...
        checkMinMaxRanges: function (input, data) {
          ...
          var rangeBytes =
            headerInfo.numDims * this.getDataTypeSize(headerInfo.imageType);
          var minValues = this.readSubArray(
            input,
            data.ptr,
            OutPixelTypeArray,
            rangeBytes
          );
          var maxValues = this.readSubArray(
            input,
            data.ptr + rangeBytes,
            OutPixelTypeArray,
            rangeBytes
        ...
        readMask: function (input, data) {
          ...
          // Mask Data
          if (
            (0 === numValidPixel || numPixels === numValidPixel) &&
            0 !== mask.numBytes
          ) {
            throw "invalid mask";
          }
        ...
        readDataOneSweep: function (input, data, OutPixelTypeArray) {
          ...
          var numBytes =
            headerInfo.numValidPixel *
            Lerc2Helpers.getDataTypeSize(imageType) *
            numDims;
          ...
          if (rawData.length === numPixels * numDims) {
            ...
            data.pixels.resultPixels = new OutPixelTypeArray(
              numPixels * numDims
            );
            var z = 0,
              k = 0,
        ...
        readHuffman: function (input, data, OutPixelTypeArray) {
          ...
          for (iDim = 0; iDim < headerInfo.numDims; iDim++) {
            if (numDims > 1) {
              ...
              resultPixels = new OutPixelTypeArray(
                resultPixelsAllDim.buffer,
                numPixels * iDim,
                numPixels
              );
              prevVal = 0;
              ...
              for (k = 0, i = 0; i < height; i++) {
                for (j = 0; j < width; j++, k++) {
                  ...
                  if (32 - bitPos < numBitsLUTQick) {
                    valTmp |=
                      stuffedData[srcPtr + 1] >>>
                      (64 - bitPos - numBitsLUTQick);
                    valTmpQuick = valTmp; // >>> deltaBits;
                  }
                  if (decodeLut[valTmpQuick]) {
                    ...
                    if (32 - bitPos < numBitsLUT) {
                      valTmp |=
                        stuffedData[srcPtr + 1] >>> (64 - bitPos - numBitsLUT);
                      valTmpQuick = valTmp; // >>> deltaBits;
                    }
                    node = tree;
        ...
        decodeBits: function (input, data, blockDataBuffer, offset, iDim) {
          {
            ...
            var blockPtr = 0;
            var viewByteLength =
              input.byteLength - data.ptr >= 5
                ? 5
                : input.byteLength - data.ptr;
        ...
        readTiles: function (input, data, OutPixelTypeArray) {
          ...
          var blockEncoding;
          var blockDataBuffer = new OutPixelTypeArray(
            microBlockSize * microBlockSize
          );
          ...
          for (blockY = 0; blockY < numBlocksY; blockY++) {
            thisBlockHeight =
              blockY !== numBlocksY - 1 ? microBlockSize : lastBlockHeight;
            for (blockX = 0; blockX < numBlocksX; blockX++) {
              ...
              outPtr =
                blockY * width * microBlockSize + blockX * microBlockSize;
              outStride = width - thisBlockWidth;

              for (iDim = 0; iDim < numDims; iDim++) {
                if (numDims > 1) {
                  resultPixels = new OutPixelTypeArray(
                    data.pixels.resultPixels.buffer,
                    width * height * iDim * dataTypeSize,
                    width * height
                  );
                }
                ...
                if (blockEncoding > 3) {
                  ...
                  //bit alignment
                  arrayBuf = new ArrayBuffer(
                    numBytes % dataTypeSize === 0
                      ? numBytes
                      : numBytes + dataTypeSize - (numBytes % dataTypeSize)
                  );
                  store8 = new Uint8Array(arrayBuf);
                  ...
                  offset = Lerc2Helpers.getOnePixel(
                    block,
                    blockPtr,
                    offsetType,
                    view
        ...
        isValidPixelValue: function (t, val) {
          ...
          switch (t) {
            ...
            case 6:
              isValid =
                val >= -3.4027999387901484e38 && val <= 3.4027999387901484e38;
              break;
            case 7:
              isValid = val >= 5e-324 && val <= 1.7976931348623157e308;
              break;
            default:
              isValid = false;
          }
      ...
      var Lerc2Decode = {
        ...
        decode: function (/*byte array*/ input, /*object*/ options) {
          ...
          var headerInfo = data.headerInfo;
          var fileVersion = headerInfo.fileVersion;
          var OutPixelTypeArray = Lerc2Helpers.getDataTypeArray(
            headerInfo.imageType
          );

          // Mask Header
          Lerc2Helpers.readMask(input, data);
          if (
            headerInfo.numValidPixel !== headerInfo.width * headerInfo.height &&
            !data.pixels.resultMask
          ) {
          ...
          if (headerInfo.numValidPixel !== 0) {
            ...
            if (headerInfo.zMax === headerInfo.zMin) {
              ...
              fileVersion >= 4 &&
              Lerc2Helpers.checkMinMaxRanges(input, data)
            ) {
              Lerc2Helpers.constructConstantSurface(data);
            } else {
              ...
              if (bReadDataOneSweep) {
                ...
                if (
                  fileVersion > 1 &&
                  headerInfo.imageType <= 1 &&
                  Math.abs(headerInfo.maxZError - 0.5) < 0.00001
                ) {
          ...
          if (
            data.pixels.resultMask &&
            Lerc2Helpers.isValidPixelValue(headerInfo.imageType, noDataValue)
          ) {
            var mask = data.pixels.resultMask;
      ...
      decode: function (encodedData, options) {
        ...
        while (inputOffset < eof) {
          ...
          if (iPlane === 0) {
            ...
            decodedPixelBlock.pixelType =
              result.pixelType || result.fileInfo.pixelType;
            decodedPixelBlock.mask = result.maskData;
          }
          if (
            majorVersion > 1 &&
            result.fileInfo.mask &&
            result.fileInfo.mask.numBytes > 0
          ) {
  ...
  var createVerticesFromHeightmap$1 = createTaskProcessorWorker(
